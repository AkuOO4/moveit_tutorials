...
This document explains the fundamental motion planning paradigms demonstrated in the `move_group_interface` tutorial. The focus is on **concepts** — what each planning task means, when to use it, and the trade-offs involved — rather than the C++ API itself.
...
7. [How These Compose in Practice](#7-how-these-compose-in-practice)
...
**The key idea:** This is the most "direct" form of motion specification — you're saying "I want joint 1 at -1.0 radians..." and so on. No IK needed because you've already told the planner exactly where every joint should be.
...
- When you need a predictable, repeatable arm shape (e.g., avoiding self-collisions or reaching into tight spaces)
...
**Why it's slow:** Without constraints, the planner can sample any joint configuration freely. With constraints, every sampled configuration must be *checked* (or projected) to satisfy the constraint. Most random samples get rejected, which is why the code bumps planning time up to 30 seconds.
...
**Always clear constraints when done:**

```cpp
move_group.clearPathConstraints();
```

Leaving path constraints active for subsequent goals will silently slow down or break unrelated plans.
...
waypoints.push_back(start_pose2);          // start
...
**The `eef_step` parameter:** This controls how finely the straight-line segments between waypoints are discretized (0.01 m = 1 cm here). Smaller steps produce smoother, more accurately linear paths but require more IK calls. Larger steps are faster but the path may deviate more from a true straight line.

**The `fraction` return value:** Cartesian planning can fail partway through if a waypoint is unreachable or the straight-line path passes through an obstacle or singularity. The planner returns the fraction of the path it successfully computed (`1.0` = full success, `0.5` = made it halfway). You decide whether partial success is acceptable.
...
- `BOX` — axis-aligned box defined by three dimensions
- `SPHERE` — sphere defined by radius
- `CYLINDER` — cylinder defined by height and radius
- `CONE` — cone defined by height and radius
...
**Async nature:** After `addCollisionObjects()`, MoveIt needs a moment to propagate the object to the `move_group` node's internal planning scene. The tutorial uses `visual_tools.prompt()` as a human-paced pause for this. In automated pipelines, you'd typically wait for the object to appear in the scene before replanning.
...
**Removing from the world:** Detaching only disconnects the object from the robot. To clean up the planning scene entirely, you must explicitly remove it.
...
## 7. How These Compose in Practice
